...
export function listsDataToOptionTags(listsData) {
    const optionTags = Object.keys(listsData).reduce((acc, key) => {
        const keyValues = listsData[key]
        acc[key] = keyValues.map((value) => (
...
export function entitiesToOptionTags(entities) {
    const optionTags = entities.reduce((acc, entity) => {
        return acc + `<option value="${entity.id}">${entity.name}</option>`
    }, "")
    return optionTags
}
