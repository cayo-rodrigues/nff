...
class Entity:
    def __init__(self, data: dict, is_sender: bool = False) -> None:
        ...
        self.ie: str = normalize_text(data.get("ie"), keep_case=True, remove=[".", "-", "/"])
        self.cpf_cnpj: str = normalize_text(data.get("cpf_cnpj"), keep_case=True, remove=[".", "-", "/"])
        self.password: str = data.get("password", "")

        self.other_ies: list[str] = [
            normalize_text(ie, keep_case=True, remove=[".", "-", "/"])
            for ie in data.get("other_ies") or []
        ]
